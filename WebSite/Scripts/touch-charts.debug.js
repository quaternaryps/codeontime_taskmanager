            ...
            if (calendar.length > 0) {
                ...
                if (!isEcho) {
                    ...
                    $app.touch.tabs('create', {
                        container: tabs,
                        tabs: [
                            { text: resourcesCalendar.Day, value: 'day', active: this.mode == 'day' },
                            { text: resourcesCalendar.Week, value: 'week', active: this.mode == 'week' },
                            { text: resourcesCalendar.Month, value: 'month', active: this.mode == 'month' },
                            { text: resourcesCalendar.Year, value: 'year', active: this.mode == 'year' },
                            { text: resourcesCalendar.Agenda, value: 'agenda', active: this.mode == 'agenda' }
                        ],
                        change: function (tab) {
                    ...
                    // save calendar footer for future reference
...
date = getDateOfBlock(visible);
    ...
    var CalendarInput = touch.CalendarInput = (function (method, options) {
        ...
        function focus() {
            var wrapper = inputContainer.closest('.app-wrapper'),
                ...
                center = windowWidth <= 640 && windowHeight <= 640,
                modal = center || touchPointer;

            if (options.date)
                outerContainer.data('select-date', options.date);

            if (center && windowWidth < windowHeight)
        ...
        function setInput(dataView, field, input, date) {
            ...
            if (touchPointer || !input || input.parent().length == 0)
                ...
                $app.input.value(field.format(date));
        }
    ...
    $(document)
        .on('focus', '.app-data-input[data-datepicker-attached]', function (e) {
            CalendarInput('focus', { input: $(e.target) });
        })
