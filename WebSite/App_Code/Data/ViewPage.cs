            ...
            else
            {
                ...
                if (IncludeMetadata("layouts"))
                {
                    ...
                    while (tryLoad)
                    {
                        ...
                        {
                            Stream stream = GetType().Assembly.GetManifestResourceStream(String.Format("MyCompany.Views.{0}.{1}.html", this.Controller, this.View));
                            if (stream != null)
                ...
                if (IncludeMetadata("items"))
                {
                    ...
                    foreach (DataField field in Fields)
                    	if (!(ApplicationServices.IsTouchClient) || ((field.ItemsStyle == "CheckBoxList") || (!(String.IsNullOrEmpty(field.ItemsTargetController)) || (configuration.SelectSingleNode("/c:dataController/c:views/c:view[@id=\'{0}\']/@type", this.View).Value == "Form"))))
